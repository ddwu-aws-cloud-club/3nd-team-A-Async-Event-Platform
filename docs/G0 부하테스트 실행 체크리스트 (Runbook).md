...
- [x]  **k6 스크립트 확인**: 매 요청마다 **랜덤 `userId` 생성** 및 **토큰 발급** 로직이 포함되어 있는지 확인 (단일 토큰 재사용 금지 -> 멱등성 필터링 방지)
- **k6 스크립트 확인**
    - **VU별로 서로 다른 `userId`를 사용**하도록 구현되어 있는지 확인
        
        (예: **`user-000001`**, **`user-000002`**, …)
        
    - **각 VU는 시작 시 1회만 로그인하여 토큰을 캐싱**하고, 이후 요청에서 재사용하는지 확인
